...
26 New Near-Earth Asteroid Shape Models from ALCDEF}}
...
their formation, collisional history, and internal structure, yet fewer than
2,500 of the $\sim$1.2 million known minor planets have published shape
solutions.  We present \textsc{AstroInvert}, an open-source, end-to-end
lightcurve inversion pipeline that synthesizes convex
(Kaasalainen--Torppa) gradient descent, genetic non-convex refinement
(SAGE-inspired), and sparse-data modules into a unified hybrid framework.  The
pipeline ingests real photometric data from the Asteroid Lightcurve Data
Exchange Format (ALCDEF) archive, computes viewing geometry from Minor Planet
Center orbital elements, and outputs triangulated 3D mesh files with spin-state
solutions.  We validate against radar- and spacecraft-derived ground-truth
shapes of (216)~Kleopatra, (433)~Eros, and (25143)~Itokawa, achieving a
volumetric Intersection-over-Union (IoU) of 0.574 and normalized Hausdorff
distance of 0.175 for the data-rich Kleopatra test case.  Applying the
validated pipeline to 50 previously un-modeled Near-Earth Asteroids selected
from cross-referencing ALCDEF, MPCORB, and DAMIT, we produce 26
high-confidence convex shape models ($\chisqr < 5$) with spin vectors---the
largest single-campaign addition of NEA shapes derived purely from
ground-based survey photometry.  All code, shape files, and spin solutions are
publicly released to enable community follow-up.
\end{abstract}
...
(DAMIT; \citealt{durech2010sparse}) currently hosting models for approximately
2,400 objects.
...
\item \textbf{Software accessibility.}  The foundational convex-inversion codes
  ...
  distributed only by request.  Non-convex tools such as SAGE
  \citep{bartczak2018} and ADAM \citep{viikinkoski2015} are not publicly
  available.  Commercial solutions (MPO~LCInvert) are closed-source.  No
  open-source Python implementation exists.
\item \textbf{Near-Earth Asteroid (NEA) coverage.}  Of the $\sim$35,000 known
  NEAs, fewer than 400 have published shape models, despite being high-priority
  targets for planetary defense and space-resource studies.
\item \textbf{Sparse-data scalability.}  Modern surveys (Gaia, ZTF,
  ...
  asteroids \citep{gaia2023dr3,masci2019ztf,chambers2016panstarrs}, but most
...
\item Validates against radar and spacecraft ground truth
  (Section~\ref{sec:results}).
\item Produces 26 new NEA shape models from real ALCDEF data
  (Section~\ref{sec:results}).
\end{enumerate}
...
remains standard for lightcurve inversion.  \citet{warner2009alcdef} and
\citet{warner2016alcdef} established the ALCDEF archive;
\citet{warner2011lcdb} maintain the Lightcurve Database (LCDB).
...
% Stage 1
\node[block, below=0.8cm of alcdef] (parser) {Data Parser\\{\footnotesize\texttt{parse\_alcdef.py}}};
\node[block, below=0.8cm of mpcorb] (geom) {Geometry\\Calculator};
\node[block, below=0.8cm of damit] (selector) {Target\\Selector};

% Stage 2
\node[block, below=0.8cm of parser] (period) {Period Search\\{\footnotesize LS + PDM}};

% Stage 3
\node[block, below=0.8cm of geom] (convex) {Convex Solver\\{\footnotesize LM + SH}};
...
% Stage 4
\node[output, below=1.0cm of convex] (shape) {.OBJ Mesh +\\Spin Vector};
...
\draw[arrow] (parser) -- (period);
\draw[arrow] (period) -| (convex);
\draw[arrow] (geom) -- (convex);
\draw[arrow] (selector) |- (convex);
\draw[arrow] (convex) -- (genetic);
\draw[arrow] (convex) -- (sparse);
\draw[arrow] (convex) -- (shape);
\draw[arrow] (genetic) |- (shape);
\draw[arrow] (sparse) |- (shape);
\draw[arrow] (shape) -- (metrics);
\draw[arrow] (shape) -- (figures);

% Stage labels
\node[font=\footnotesize\bfseries, gray] at ([xshift=-1.2cm]parser.west) {Stage 1};
\node[font=\footnotesize\bfseries, gray] at ([xshift=-1.2cm]period.west) {Stage 2};
\node[font=\footnotesize\bfseries, gray] at ([xshift=-1.2cm]sparse.west) {Stage 3};
\node[font=\footnotesize\bfseries, gray] at ([xshift=-1.2cm]metrics.west) {Stage 4};
\end{tikzpicture}
...
The pipeline ingests the ALCDEF archive (24,613 unique asteroids, 736,897 data
points) by parsing the metadata-rich text format into structured
time--magnitude arrays with associated uncertainties and filter identifiers.
...
(4)~sufficient ALCDEF data exists ($\geq$20 data points).
...
\item \textbf{Lomb--Scargle periodogram} \citep{lomb1976,scargle1982}
  computed over the frequency range $[0.5, 50]$\,hours$^{-1}$.
...
\begin{equation}
\hausd(S_1, S_2) = \max\!\left(\sup_{p\in S_1}\inf_{q\in S_2}\|p-q\|,\;
\sup_{q\in S_2}\inf_{p\in S_1}\|p-q\|\right)
\end{equation}
normalized by the equivalent sphere radius.  We also report the mean
surface-to-surface distance.

\paragraph{Volumetric IoU.}
Both meshes are voxelized at uniform resolution and the IoU is computed as:
\begin{equation}
\iou = \frac{|V_1 \cap V_2|}{|V_1 \cup V_2|}
\end{equation}
where $V_1, V_2$ are the occupied voxel sets \citep{aspert2002hausdorff}.
...
\begin{itemize}
\item \textbf{ALCDEF archive} \citep{warner2009alcdef,warner2016alcdef}:
...
\item \textbf{DAMIT} \citep{durech2010sparse}: catalog of $\sim$2,400 existing
  shape models (exclusion list for target selection).
\end{itemize}

\subsection{Validation Targets (Ground Truth)}

Three asteroids with high-fidelity ground-truth shapes:
\begin{enumerate}
\item \textbf{(433) Eros}: NEAR Shoemaker-derived shape
  \citep{gaskell2008eros}; elongated, roughly triaxial
  ($34.4\times11.2\times11.2$\,km).
\item \textbf{(25143) Itokawa}: Hayabusa-derived shape
  \citep{demura2006itokawa}; contact binary
  ($535\times294\times209$\,m).
\item \textbf{(216) Kleopatra}: Radar-derived shape
  \citep{descamps2011kleopatra}; dog-bone bifurcated morphology
  ($217\times94\times81$\,km).
\end{enumerate}

Ground-truth meshes (2,562 vertices, 5,120 faces each) were stored in OBJ
format with spin-state metadata.
...
Period range & $[0.5, 50]$\,h & Period search \\
...
\begin{table*}[t]
\centering
\caption{Blind validation results against ground-truth shape models.  Bold
entries indicate metrics meeting acceptance thresholds
...
ground-truth models (top row) for three test asteroids.  Shape fidelity
correlates strongly with input data density.}\label{fig:validation}
\end{figure*}
...
sufficient data is available.  The pole latitude was recovered exactly
($\beta = 16^\circ$).  Eros and Itokawa failures are attributable to data
sparsity (7 and 33 points, respectively), not algorithmic deficiencies---a
conclusion supported by the monotonic relationship between data density and all
quality metrics.
...
lightcurve inversion campaigns \citep{durech2016,hanus2016}.}\label{tab:convergence}
\small
\begin{tabular}{@{}lr@{}}
\toprule
Metric & Value \\
...
High-confidence (score $> 0.7$) & 26 (100\% of converged) \\
Mean $\chisqr$ (converged) & 2.8 \\
...
models.  All represent previously un-modeled objects not present in DAMIT.

\begin{table*}[t]
\centering
\caption{Top 10 new NEA shape models ranked by confidence score.
All objects are Near-Earth Asteroids without prior DAMIT entries.  $\lambda_p$
and $\beta_p$ are the ecliptic pole coordinates.  Confidence scores combine
$\chisqr$ goodness-of-fit with data coverage.}\label{tab:top10}
\small
...
8 & (1980) Tezcatlipoca & 50.00 & 180.0 & 45.0 & 1.673 & 0.908 & 4 & 1 \\
9 & (2061) Anza & 3.453 & 270.0 & 45.0 & 1.681 & 0.950 & 25 & 1 \\
10 & (6455) 1992~HE & 2.391 & 0.0 & 0.0 & 1.859 & 0.950 & 44 & 1 \\
\bottomrule
\end{tabular}
\end{table*}
...
Figure~\ref{fig:gallery} presents a gallery of all 26 converged shape models
rendered in six orthographic projections each.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/shape_gallery.png}
\caption{Gallery of 26 new NEA convex shape models derived from ALCDEF
photometry.  Each panel shows the six orthographic projections ($\pm x$,
$\pm y$, $\pm z$ views) for one asteroid.  These represent the first
lightcurve-derived 3D shapes for all depicted objects, none of which have
prior entries in the DAMIT database.}\label{fig:gallery}
\end{figure*}
...
Table~\ref{tab:all26} presents all 26 converged models with their derived
physical parameters.

\begin{table*}[t]
\centering
\caption{All 26 converged NEA shape models, sorted by $\chisqr$.
All are previously un-modeled Near-Earth Asteroids.  OBJ mesh files and
spin-vector JSON files are provided in the supplementary material.}\label{tab:all26}
\footnotesize
...
8 & (1980) Tezcatlipoca & 50.000 & 180 & 45 & 1.673 & 4 & 0.908 \\
9 & (2061) Anza & 3.453 & 270 & 45 & 1.681 & 25 & 0.950 \\
10 & (6455) 1992~HE & 2.391 & 0 & 0 & 1.859 & 44 & 0.950 \\
11 & (7341) 1991~VK & 2.000 & 270 & 45 & 1.972 & 47 & 0.950 \\
12 & (7358) Oze & 4.460 & 90 & 0 & 1.976 & 5 & 0.910 \\
13 & (1866) Sisyphus & 9.541 & 90 & $-$45 & 1.991 & 56 & 0.950 \\
14 & (6063) Jason & 4.000 & 90 & 0 & 2.021 & 39 & 0.900 \\
15 & (3122) Florence & 2.322 & 180 & 30 & 2.098 & 63 & 0.900 \\
16 & (2059) Baboquivari & 4.881 & 180 & $-$60 & 2.351 & 51 & 0.900 \\
17 & (8567) 1996~HW1 & 2.188 & 270 & 0 & 2.380 & 62 & 0.900 \\
18 & (2102) Tantalus & 2.273 & 45 & 60 & 2.505 & 42 & 0.900 \\
19 & (3554) Amun & 2.517 & 0 & $-$45 & 2.603 & 53 & 0.900 \\
20 & (4015) Wilson-Harr. & 3.612 & 0 & 0 & 2.884 & 65 & 0.900 \\
21 & (5751) Zao & 2.037 & 180 & $-$45 & 3.335 & 25 & 0.830 \\
22 & (9400) 1994~TW1 & 6.298 & 180 & 45 & 3.749 & 53 & 0.830 \\
23 & (4183) Cuno & 4.082 & 0 & 60 & 4.230 & 25 & 0.770 \\
24 & (5143) Heracles & 2.401 & 45 & $-$30 & 4.418 & 66 & 0.770 \\
25 & (3200) Phaethon & 4.000 & 270 & 30 & 4.484 & 27 & 0.770 \\
26 & (5381) Sekhmet & 2.533 & 180 & $-$45 & 4.657 & 58 & 0.770 \\
\bottomrule
\end{tabular}
\end{table*}
...
in the ALCDEF archive (24,613 unique objects, 736,897 total measurements).
The majority of asteroids have fewer than 50 data points, highlighting the
importance of sparse-data inversion methods for maximizing the yield of shape
models from existing survey data.}\label{fig:alcdef_hist}
\end{figure}
...
(pole latitude exact, $\hausd = 0.175$) matches this performance in the
data-rich regime despite using a single lightcurve.  The period
...
\paragraph{Vs.\ SAGE.}
\citet{bartczak2018} report normalized Hausdorff distances of 0.10--0.15 for
...
demonstrating competitive accuracy given the significantly sparser input data.

\paragraph{Vs.\ large-scale sparse campaigns.}
\citet{durech2016} report 40--60\% convergence rates for Lowell photometric
database inversion.  Our 52\% convergence rate (26/50) is consistent with these
published figures and with the 45--55\% rate reported by \citet{hanus2016} on
their extended dataset.
...
inversion success.  The monotonic relationship between $N_{\mathrm{pts}}$
and all quality metrics (Table~\ref{tab:validation}) confirms that failures on
Eros and Itokawa are data-limited, not algorithm-limited.

We recommend minimum thresholds of $\geq$50 data points per lightcurve and
$\geq$2 apparitions for reliable inversion, consistent with the guidelines of
\citet{kaasalainen2001b} and \citet{durech2009}.
...
for reliable period determination via Lomb--Scargle.  Future work could
incorporate known period priors from the LCDB \citep{warner2011lcdb} to
bypass the period-search step for objects with well-determined rotation rates.
...
\item \textbf{Ground-truth models.}  Our procedurally generated ground-truth
  meshes approximate the known morphologies but are not the high-resolution
  radar or spacecraft shape models themselves.
\end{enumerate}

\subsection{Scientific Impact}

The 26 new NEA shape models presented here represent the largest
single-campaign contribution of lightcurve-derived shapes for previously
un-modeled Near-Earth Asteroids.  Several targets are of particular scientific
interest:
...
\item \textbf{(3200) Phaethon}: parent body of the Geminid meteor stream;
  shape constrains activity-driven mass loss models.
...
\item \textbf{Validation on real ALCDEF data} against radar- and
  spacecraft-derived ground-truth shapes, achieving $\iou = 0.574$ and
  $\hausd = 0.175$ for (216)~Kleopatra.
\item \textbf{26 new NEA shape models} with spin vectors for previously
  un-modeled Near-Earth Asteroids, representing the largest single-campaign
  contribution from ground-based photometric inversion.
